# Приятелски класове и функции. Ключова дума auto. Функции от по-висок ред. Предефиниране на оператори. 
...
- Може да се напишат в private, protected, public частта на класа (Добрата практика е само в public)
...
private:
    ...
    int num2 = 2;
public:
    friend void f(const Example& e);
    friend int g(const Example& e);
};

void f(const Example& e)
{
    std::cout << e.num1 << std::endl;
}

int g(const Example& e)
{
...
## Ключова дума auto.
Ключовата дума **auto** в C++ се използва при декларации на променливи или функции. Тя указва, че типът на декларираната променлива ще бъде автоматично изведен (deduced) от нейния инициализатор.

В случай на функции, когато типът на връщаната стойност е auto, той се определя по време на компилация на база на return израза.

**auto** се използва често, за да се избегне писането на дълги и сложни типове - например при итератори или сложни изрази.

**Важно:**
- auto винаги изисква инициализация
- премахват се top-level const и reference, освен ако не използваме auto&
...
auto calculate(double a, int b)
{
    return a * b + 2.5; // double
}
...
{
    auto s = sum(3, 4);
    auto c = calculate(2.5, 2);
    auto m = multiply(5, 6);

    cout << "sum result: " << s << endl;
    cout << "calculate result: " << c << endl;
    cout << "multiply result: " << m << endl;

    cout << endl;

    cout << "Type of sum: " << typeid(s).name() << endl;
    cout << "Type of calculate: " << typeid(c).name() << endl;
    cout << "Type of multiply: " << typeid(m).name() << endl;
...
int main()
{
    ...
    {
        point.x += 100; // променя САМО копието
    }

    cout << "After auto (copy):" << endl;
    for (auto point : points)
    {
    ...
    for (auto& point : points)
    {
        point.x += 100; // променя оригиналните обекти
    }

    cout << "After auto& (reference):" << endl;
...
В C++ функциите от по-висок ред могат да се реализират чрез:
- указатели към функции
- ламбда функции
- обекти с предефиниран operator() (functors)
- std::function (модерен и най-гъвкав подход)

В съвременния C++ най-често се използват ламбда функции и std::function,
тъй като те позволяват по-голяма гъвкавост и по-четим код.
...
int calculate(int a, int b, int (*operation)(int, int)) // operation е указател към функция
{
    return operation(a, b);
}

int calculate(int a, int b, std::function<int(int, int)> operation) // модерния начин на горното
{
    return operation(a, b);
}
...
int main()
{
    cout << calculate(3, 4, add) << endl; // подаваме поведение като параметър
    cout << calculate(3, 4, multiply) << endl;
...
### Ламбда функции
Ламбда функция е анонимна функция (без име), която можем да създадем директно на място.
```c++
[capture](parameters) -> return_type {
    // тяло
}
```
- ```[]``` – capture списък (какви външни променливи използваме)
- ```()``` – параметри
- ```{}``` – тяло на функцията
- ```-> return type``` - типът на връщане, ако не го напишем се извежда автоматично (auto)
...
{
    // с lambda
    cout << calculate(3, 4, [](int a, int b) {
        return a * b;
    }) << endl;

    // lambda с capture
    int bonus = 10;
    cout << calculate(3, 4, [bonus](int a, int b) {
        return a + b + bonus;
    }) << endl;
...
Ако един предефиниран оператор бива дефиниран като член-функция на класа, то лявата операнда бива директно асоциирана с обекта, върху който се извиква функцията, т.е. с **this**, като в този случай оператора има с един по-малко параметри.  
...
### Операторите се характеризират с:
 ...
 -  **Позиция на оператора спрямо аргумента** 
    - **префиксен** -> ++a, !a, *a
    - **инфиксен** -> a + b
    - **суфиксен** -> a++
...
В рамките на курса ще ги дефинираме извън класа, т.е. не като член-функции, т.к. те не налагат промяна върху лявата операнда (в случая - **this**), а единствено връщат нова стойност. Тези операции **НЕ** променят състоянията на която и да е от операндите, затова те се подават като референция към константа.  
Те обикновено генерират нова стойност, която е резултатът в следствие на използване на съответната аритметична операция. Тази стойност се държи в локална променлива, която се връща в края на функцията.  
...
{
	MyDouble temp(lhs);        //create a local copy of lhs, calling the copy constructor of MyDouble
...
### 3-way comparison operator (Space Ship Operator)
Операторът <=> е въведен в C++20 и се използва за сравнение между два обекта.

Той връща резултат, който показва дали:
- левият операнд е по-малък
- равен
- или по-голям от десния
...
```
Операторът връща специален тип от <compare>:
- std::strong_ordering - ```5 == 5```
- std::weak_ordering - ```"abc" == "ABC"```
- std::partial_ordering - ```NaN <=> 3.14```

За базови типове обикновено се използва ```std::strong_ordering```.
...
int main()
{
    ...
    if (x == y) // не работи, ако не сме писали operator==
        cout << "equal" << endl;
...
```
При дефинирането на операторите == и <=>, компилаторът може да генерира всички останали оператори за сравнение
